...
class DBPools:
    ...
    def get_pool(db_name):
        if db_name not in DBPools.pools:
            config = configdb(section=db_name)
            DBPools.pools[db_name] = pool.ThreadedConnectionPool(1, 50, **config)
        return DBPools.pools[db_name]

@contextmanager
def connection(db_name='postgresql'):
    con = None
    cur = None
    try:
        pg_pool = DBPools.get_pool(db_name)
        con = pg_pool.getconn()
        cur = con.cursor()
        yield cur
        con.commit()
